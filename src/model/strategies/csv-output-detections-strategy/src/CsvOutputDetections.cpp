                ...
                for (const auto &detection: sensor_data.feature_data().lidar_sensor(sensor_idx).detection()) {
                    ...
                    if (detection.has_intensity())
                        write_data_to_CSV(file_path_detections, timestamp, detection_idx,
                                          ...
                                          detection.position().distance(), detection.intensity());
                    else if (detection.has_echo_pulse_width())
                        write_data_to_CSV(file_path_detections + "\\Detections.csv", timestamp, detection_idx,
                                          detection.position().azimuth() * 180 / M_PI,
                                          detection.position().elevation() * 180 / M_PI,
                                          detection.position().distance(), detection.echo_pulse_width());
                    detection_idx++;
                }
            }
        }
